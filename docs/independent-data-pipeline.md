...
A full Hero/market rebuild writes both `site/public/data/wallet-explorer.json` and the much smaller `site/public/data/top-holders.json`. The full Wallet Explorer index is a deterministic projection of current beneficial ownership, staking/quest timestamps, rarity, public-mint provenance, and validated supported-market buyer/seller records. The top-holder product is derived from that same ownership state and contains only the ten ranked shortcut rows needed on the main Ownership section. `scripts/validate_live.py` cross-checks both products against the primary dashboard data before release.

The full public Wallet Explorer artifact remains one compact all-wallet index rather than address-specific files or an API. The dedicated Wallet Explorer view downloads it only when opened and resolves entered addresses in the browser; the normal Analytics page downloads only the tiny top-holder projection. This keeps wallet searches out of server/provider request logs while still allowing saved address sets to remain local browser preferences. Market cash-flow presentation is explicitly market-only: sales without a visible prior supported-market purchase in the selected wallet view are retained in history but excluded from matched received/net totals.
